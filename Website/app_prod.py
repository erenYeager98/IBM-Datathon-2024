...
# Load ONNX model
onnx_model_path = 'postpartum.onnx'
...
def ppd_test_result():
    ...
    # Convert the DataFrame to a NumPy array with the required shape
    input_data = df.to_numpy().astype(np.float32)

    # Perform the ONNX model prediction
    ...
    # Assuming the model output is binary (0 for negative, 1 for positive)
    if output[0] > 0.5:
        print('Positive')
